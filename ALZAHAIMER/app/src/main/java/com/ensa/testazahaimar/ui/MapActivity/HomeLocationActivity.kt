...
class HomeLocationActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    private fun fetchHomeLocation() {
        val url = "http://10.0.2.2:3000/api/users/$userId/home-location" // Ensure the URL is correct

        val jsonObjectRequest = JsonObjectRequest(
            Request.Method.GET, url, null,
            Response.Listener { response ->
                Log.d("HomeLocationActivity", "Response: $response")
                ...
                if (response.has("latitude") && response.has("longitude")) {
                    ...
                    Log.d("HomeLocationActivity", "Home location retrieved: $homeLocation")

                    // Check if mMap is initialized and then display the home location
                    mMap?.let {
                        it.addMarker(MarkerOptions().position(homeLocation!!).title("Home Location"))
                        it.moveCamera(CameraUpdateFactory.newLatLngZoom(homeLocation!!, 15f))
                    } ?: Log.e("HomeLocationActivity", "mMap is null when trying to show home location")
                } else {
                    Log.e("HomeLocationActivity", "Home location coordinates are missing")
                }
            },
            Response.ErrorListener { error: VolleyError ->
                Log.e("HomeLocationActivity", "Error fetching home location: ${error.message}")
            }
        )
        ...
        if (ActivityCompat.checkSelfPermission(this, Manifest.permission.ACCESS_FINE_LOCATION) == PackageManager.PERMISSION_GRANTED) {
            fusedLocationClient.lastLocation.addOnSuccessListener { location: Location? ->
                if (location != null) {
                    ...
                    Log.d("HomeLocationActivity", "Current location retrieved: $currentLocation")
                    ...
                    mMap?.let {
                        ...
                        homeLocation?.let { homeLoc ->
                            drawRoute(currentLocation!!, homeLoc)
                        }
                    } ?: Log.e("HomeLocationActivity", "mMap is null when trying to show current location")
                } else {
                    Log.e("HomeLocationActivity", "User location not available")
                }
            }
        } else {
            ActivityCompat.requestPermissions(this, arrayOf(Manifest.permission.ACCESS_FINE_LOCATION), LOCATION_PERMISSION_REQUEST_CODE)
        ...
        val jsonObjectRequest = JsonObjectRequest(
            ...
            Response.Listener { response ->
                try {
                    ...
                    Log.e("HomeLocationActivity", "Error fetching directions: $e")
                }
            },
            Response.ErrorListener { error ->
                Log.e("HomeLocationActivity", "Error fetching directions: $error")
            }
        )
